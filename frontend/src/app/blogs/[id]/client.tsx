    ...
    <main className="min-h-screen transition-colors duration-300 bg-slate-50 dark:bg-slate-950">
      ...
      <div className="max-w-4xl px-4 py-12 mx-auto sm:px-6 lg:px-8">
        ...
        >
          <Link
            href="/blogs"
            className="inline-flex items-center gap-2 text-sm font-medium text-blue-600 transition-colors dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300"
          >
            <ArrowLeft size={16} />
            Back to Blogs
          </Link>
        ...
        >
          {/* Category Badge */}
          <div className="mb-4">
            <span className="inline-block px-4 py-1 text-sm font-medium text-blue-700 bg-blue-100 rounded-full dark:bg-blue-500/20 dark:text-blue-400">
              {blog.category}
            </span>
          </div>
        ...
        >
          {blog.tags.map((tag) => (
            <span
              key={tag}
              className="inline-block px-3 py-1 text-sm font-medium border rounded-full border-slate-300 dark:border-white/20 bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-gray-400"
            >
              {tag}
            </span>
          ))}
        ...
        <motion.article
          ...
          className="mb-12 prose dark:prose-invert max-w-none"
        >
            ...
            {blog.content.split('\n\n').map((paragraph, idx) => {
              if (paragraph.startsWith('##')) {
                ...
                return (
                  <h2
                    key={idx}
                    className="mt-8 mb-4 text-2xl font-bold text-slate-900 dark:text-slate-100"
                  >
                    {heading}
                  </h2>
                );
        ...
        >
          {prevBlog ? (
            <Link href={`/blogs/${prevBlog.id}`} className="group">
              <div className="p-6 transition-all duration-300 border rounded-lg glass-card hover:shadow-lg">
                ...
                <h3 className="text-lg font-semibold transition-colors text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400">
                  {prevBlog.title}
                </h3>
              </div>
            </Link>
          ...
          {nextBlog ? (
            <Link href={`/blogs/${nextBlog.id}`} className="group md:ml-auto md:col-span-1">
              <div className="p-6 transition-all duration-300 border rounded-lg glass-card hover:shadow-lg">
                <div className="flex items-center justify-end gap-2 mb-3 text-sm text-blue-600 dark:text-blue-400">
                  Next
                  <ArrowRight size={16} />
                </div>
                <h3 className="text-lg font-semibold text-right transition-colors text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400">
                  {nextBlog.title}
                </h3>
              </div>
            </Link>
              ...
              {relatedBlogs.map((relatedBlog) => (
                <Link key={relatedBlog.id} href={`/blogs/${relatedBlog.id}`} className="group">
                  <div className="overflow-hidden transition-all duration-300 rounded-lg glass-card hover:shadow-lg">
                    ...
                    <div className="p-4">
                      <span className="inline-block px-2 py-1 mb-2 text-xs font-medium text-blue-700 bg-blue-100 rounded dark:bg-blue-500/20 dark:text-blue-400">
                        {relatedBlog.category}
                      </span>
                      <h3 className="mb-2 font-semibold transition-colors text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 line-clamp-2">
                        {relatedBlog.title}
                      </h3>
                      <p className="text-sm text-slate-600 dark:text-gray-400 line-clamp-2">
                        {relatedBlog.description}
                      </p>
                      <div className="flex items-center gap-2 mt-4 text-xs text-slate-600 dark:text-gray-500">
                        <Calendar size={14} />
                        {relatedBlog.date}
                      </div>
                    </div>
